...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "SELECT * FROM users WHERE Email='$email' AND verify_token='$verification_code' ";
    $result = mysqli_query($conn, $sql);

    if (mysqli_num_rows($result) > 0) {
        // Update the user's record to mark them as verified
        $update_sql = "UPDATE users SET Status='Verified' WHERE Email='$email' AND verify_token='$verification_code'";
        if (mysqli_query($conn, $update_sql)) {
            // Redirect the user to a page indicating successful verification
            $_SESSION['auth_status'] = "You have successfully verified. You can now login your account";
            header("Location: Loginform.php?success=you have successfully verified");
            exit();
        } else {
            // Display an error message if the update fails
            $_SESSION['auth_status'] = "Verification failed, please try again";
        ...
        $_SESSION['auth_status'] = "Invalid verification code, please try again";
    ...
    $_SESSION['auth_status'] = "You cannot access this site directly";
